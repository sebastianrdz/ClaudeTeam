...
|---|---|---|
| Engineering / Frontend | `frontend-lead` | `react-engineer`, `motion-engineer`, `ui-stylist` |
...
3. **Plan.** Break the mission into work packages. Each package gets: objective, context brief, constraints, explicit **acceptance criteria**, and an output contract (what the lead must return). UI packages also get a **craft directive** — the one skill the surface is built under, from `docs/DESIGN-SKILLS.md` (`impeccable` for product surfaces, `design-taste-frontend` for landing pages, the Emil pack for motion and component feel). Two direction-setting skills on one surface produce mush.
